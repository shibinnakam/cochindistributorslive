  ...
  router.post('/add', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      if (newQuantity > maxQty) {
        return res.status(400).json({ msg: `Maximum quantity per product is ${maxQty}` });
      }
      ...
      if (newQuantity > product.quantity) {
        return res.status(400).json({ msg: `Only ${product.quantity} units available in stock` });
      }
      ...
      io.to(req.user.id).emit('cartUpdated', { cart: user.cart });
